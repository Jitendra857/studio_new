...
{
    public class InqueryStatusController : BaseController
    {
        DbConnection ObjDBConnection = new DbConnection();
        ProductHelpers objProductHelper = new ProductHelpers();
        private readonly IWebHostEnvironment _hostingEnvironment;
        MasterDropdownHelper master = new MasterDropdownHelper();
        public InqueryStatusController(IWebHostEnvironment hostingEnvironment)
        {
        ...
        public PartialViewResult InqueryReport(ReportmodelParameterpost reportmodel)
        {
        ...
        public JsonResult InqueryReportexcel(ReportmodelParameterpost rm)
        {
            List<InquerystatusReportModel> Inquery = new List<InquerystatusReportModel>();

            var sqlParameters = new SqlParameter[7];
            sqlParameters[0] = new SqlParameter("@InqFromDate", rm.inqFromDate);
            sqlParameters[1] = new SqlParameter("@InqToDate", rm.inqToDate);
            sqlParameters[2] = new SqlParameter("@EventFromDate", rm.eventFromDate);
            sqlParameters[3] = new SqlParameter("@EventToDate", rm.eventToDate);
            sqlParameters[4] = new SqlParameter("@Customer", rm.customer);
            sqlParameters[5] = new SqlParameter("@InqStatus", rm.InqStatus);
            sqlParameters[6] = new SqlParameter("@orderby", rm.sortorder);
            DataTable dt = ObjDBConnection.CallStoreProcedure("GetInqueryRegisterReport", sqlParameters);
            if (dt.Rows.Count > 0)
            {
        ...
        private PdfPTable Add_Content_To_PDF(PdfPTable tableLayout, List<InquerystatusReportModel> Inquery,string layout, ReportmodelParameterpost reportmodel)
        {
            ...
            // Customer details
            tableLayout.AddCell(new PdfPCell(new Phrase("Date : " + reportmodel.inqFromDate +" To "+ reportmodel.inqToDate, new Font(Font.FontFamily.HELVETICA, 9, 0, new iTextSharp.text.BaseColor(System.Drawing.Color.Black)))) { Colspan = 7, Border = 0,PaddingTop=3, PaddingBottom = 5, HorizontalAlignment = Element.ALIGN_LEFT });
            ...
            AddCellToBodyBold(tableLayout, "Inq No");
            AddCellToBodyBold(tableLayout, "Inq Date");

	...
	public class InquiryTransactionController : BaseController
	{
		...
		public IActionResult Index(long id,string type=null)
		{
			...
			else
			{
				...
				model.InquiryNo = objProductHelper.GetLstInqNo(2, companyId) + 1;
				model.InquiryDate = DateTime.Now.ToString("dd/MM/yyyy");
			}

		 model.lstCustomer = objProductHelper.GetCustomerList(companyId);
			
			model.lstInqStatus = master.GetDropgen("Statustop3");
		...
		{
			int companyId = Convert.ToInt32(GetIntSession("CompanyId"));
			var customerList = objProductHelper.GetCustomerListOnly(companyId);
		...
		public ActionResult Index(long id, InqueryMasterModel obj)
		{
			...
			try
			{
				...
				{
					if (obj.InquiryDate != null && obj.InquiryDate != "")
					{
						string xDate = obj.InquiryDate.ToString().Substring(6, 4) + "-" + obj.InquiryDate.ToString().Substring(3, 2) + "-" + obj.InquiryDate.ToString().Substring(0, 2);
						obj.InquiryDate = xDate;

                    }
					else obj.InquiryDate = null;
					...
					sqlParameters[16] = new SqlParameter("@CmpVou", companyId);
		...
		public PartialViewResult HeaderFotterLoad(long id = 0)
		{
			...
			if (DtEmp != null && DtEmp.Rows.Count > 0)
			{
				...
				if (DtEmp.Rows[0]["InqSubj"].ToString() == "")
				{
					SqlParameter[] sqlParameters01 = new SqlParameter[2];
					sqlParameters01[0] = new SqlParameter("@inqid", (id-1));
					sqlParameters01[1] = new SqlParameter("@flag", 2);
					DataTable DtEmp01 = ObjDBConnection.CallStoreProcedure("Update_Inq_sub_Mob_Foot", sqlParameters01);
					model.InqId = Convert.ToInt32(id);
					if (DtEmp01.Rows.Count > 0)
					{
						//model.Subject = DtEmp01.Rows[0]["InqSubj"].ToString();
						//model.Hedaer = DtEmp01.Rows[0]["InqHeader"].ToString();
						model.Mobile = DtEmp01.Rows[0]["InqMobile"].ToString();
						model.Footer = DtEmp01.Rows[0]["InqFooter"].ToString();
					}
				}
			}
		...
		public List<InqueryReportModel> getReportcontent(long id)
		{
			...
			if (ds.Tables[0].Rows.Count > 0)
			{
				...
				foreach (DataRow item in ds.Tables[0].Rows)
				{
					...
					inq.IntEveHrs = item["IntEveHrs"].ToString();
		...
		public IActionResult InqPdf(int id)
		{
			//1047
           ...
           string customSwitches = $"--print-media-type --footer-right \"Page [page] of [topage]\" --allow {footerUrl} --footer-html {footerUrl} --footer-spacing 0 --header-html {headerUrl}";
            ...
            return new ViewAsPdf("InqPdf",data)
			{
				PageSize = Size.A4,
			...
			//var data = getReportcontent(985);
			//string headerUrl = Url.Action("Header", "Home", null, Request.Scheme);
			//string footerUrl = Url.Action("Footer", "Home", null, Request.Scheme);

			//// Custom switches for Rotativa options
			//string customSwitches = $"--print-media-type --page-offset 2 --footer-center [page] --allow {footerUrl} --footer-html {footerUrl} --footer-spacing -10 --header-html {headerUrl}";

			//return new ViewAsPdf("DemoViewAsPDF", data)
			//{
			//	PageSize = Size.A4,
			//	PageOrientation = Orientation.Portrait,
			//	CustomSwitches = customSwitches,
		...
		private PdfPTable Add_Content_To_PDF(PdfPTable tableLayout, List<InqueryReportModel> Inquery, string Headertype)
		{
			...
			var uniqu = Inquery.Select(p => p.IntEvnNm).Distinct();
			...
			// Customer details
			tableLayout.AddCell(new PdfPCell(new Phrase("Date : " + Inquery[0].InqDt, new Font(Font.FontFamily.HELVETICA, 9, 1, new iTextSharp.text.BaseColor(System.Drawing.Color.Black)))) { Colspan = 2, Border = 0, PaddingBottom = 6, HorizontalAlignment = Element.ALIGN_LEFT });
			...
			tableLayout.AddCell(new PdfPCell(new Phrase(Inquery[0].AccNm, new Font(Font.FontFamily.HELVETICA, 10, 1, new iTextSharp.text.BaseColor(System.Drawing.Color.Black)))) { Colspan = 5, Border = 0, PaddingBottom = 5, HorizontalAlignment = Element.ALIGN_LEFT });
			...
			//tableLayout.AddCell(new PdfPCell(new Phrase(Inquery[0].header, new Font(Font.FontFamily.HELVETICA, 10, 1, BaseColor.BLACK))) { Colspan = 3, Border = 0, BorderWidthBottom = 1, Padding = 1, BorderColor = BaseColor.BLACK, PaddingBottom = 2, HorizontalAlignment = Element.ALIGN_CENTER });
			tableLayout.AddCell(new PdfPCell(new Phrase("", new Font(Font.FontFamily.HELVETICA, 11, 1, BaseColor.BLACK))) { Colspan = 1, Border = 0, PaddingBottom = 2, HorizontalAlignment = Element.ALIGN_CENTER });
			...
			foreach (var item in uniqu.ToList())
			{
                ...
                {
					AddCellToBodyNobold(tableLayout, "Venue To:");
					AddCellToBody(tableLayout, Inquery1[0].VenueToAddOne);
					AddCellToBody(tableLayout, "");
					AddCellToBody(tableLayout, "");
					AddCellToBody(tableLayout, "");
				}

...
namespace Soc_Management_Web.Controllers
{
    public class OrderController : BaseController
    {
        ...
        public IActionResult Index(long id = 0, int inqid = 0,int orderid=0)
        {
            ...
            if (id > 0)
            {
                ...
                {
                    string inputDate = DtEmp.Rows[0]["OrdDt"].ToString();
            ...
            else if (inqid > 0)
            {
                ...
                sqlParameters1[2] = new SqlParameter("@CmpVou", companyId);
            ...
            else
            {
                ...
                sqlParameters1[1] = new SqlParameter("@CmpVou", companyId);
                DataTable DtEmp1 = ObjDBConnection.CallStoreProcedure("DeleteDuplicaterecord", sqlParameters1);
                model.OdrNo = (objProductHelper.GetSlNo(3, companyId) + 1).ToString();
                model.OrderDate = DateTime.Now.ToString("dd/MM/yyyy");
            }

            model.lstCustomer = objProductHelper.GetCustomerListOnly(companyId);
            model.lstInqStatus = master.GetDropgen("Status",companyId);
            model.lstExtraitem = objProductHelper.GetlstExtraItemsDropdown(companyId);
            model.lstReference = objProductHelper.GetlstRefByDropdown(companyId);
            model.lstTermsAndCondition = objProductHelper.GetlstTearmAndConditionDropdown(companyId);

            model.lstJobmaster = objProductHelper.GetJobMasterLst(companyId);
            model.EventLst = objProductHelper.GetEventLst(companyId);
            model.InquiryNo = inqid;
            return View(model);
        }
        ...
        public async System.Threading.Tasks.Task<ActionResult> IndexAsync(long id, OrderModel obj)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(Convert.ToString(obj.InquiryNo)))
                {
                    ...
                    if (obj.OrderDate != null && obj.OrderDate != "")
                    {
                        string xDate = obj.OrderDate.ToString().Substring(6, 4) + "-" + obj.OrderDate.ToString().Substring(3, 2) + "-" + obj.OrderDate.ToString().Substring(0, 2);
                        obj.OrderDate = xDate;

                    }
                    else obj.OrderDate = null;
                    ...
                    sqlParameters[17] = new SqlParameter("@CmpVou", companyId);
        ...
        public ActionResult DeleteFile (int OrderId)
        {
            int companyId = Convert.ToInt32(GetIntSession("CompanyId"));
            SqlParameter[] sqlParameters = new SqlParameter[3];
            sqlParameters[0] = new SqlParameter("@Flg", 6);
            sqlParameters[1] = new SqlParameter("@ordVou", OrderId);
            sqlParameters[2] = new SqlParameter("@CmpVou", companyId);
            DataTable DtCat = ObjDBConnection.CallStoreProcedure("usp_OrderMst_Insert", sqlParameters);
            int ErrorCode = 0;
            string Message = "";
            if (DtCat != null && DtCat.Rows.Count > 0)
            {
                ErrorCode = Convert.ToInt32(DtCat.Rows[0]["ErrorCode"].ToString());
                Message = Convert.ToString(DtCat.Rows[0]["Message"].ToString());
                
            }

                return Json(new { ErrorCode, Message });
        }
        ...
        {
            int companyId = Convert.ToInt32(GetIntSession("CompanyId"));
            string xDate = data.ocasiondate.ToString().Substring(6, 4) + "-" + data.ocasiondate.ToString().Substring(3, 2) + "-" + data.ocasiondate.ToString().Substring(0, 2);
            data.ocasiondate = xDate;
            string xToDate = data.Todate.ToString().Substring(6, 4) + "-" + data.Todate.ToString().Substring(3, 2) + "-" + data.Todate.ToString().Substring(0, 2);
            data.Todate = xToDate;
            ...
            sqlParameters[6] = new SqlParameter("@ocasiondate", data.ocasiondate);
            ...
            sqlParameters[21] = new SqlParameter("@Todate", data.Todate);
            ...
            sqlParameters[29] = new SqlParameter("@CmpVou", companyId);
        ...
        public JsonResult GetJobDetails(OrderJob data)
        {
            ...
            if (PrDtEmp != null && PrDtEmp.Rows.Count > 0)
            {
                for (var i = 0; i < PrDtEmp.Rows.Count; i++)
                {
                    ...
                    obj.FromDate = ((DateTime)PrDtEmp.Rows[i]["FromDate"]).ToString("dd/MM/yyyy");
        ...
        public JsonResult GetOrderDetails(GetAlldetails data)
        {
            ...
            for (int i = 0; i < DtEmp.Rows.Count; i++)
            {
                ...
                dt.remarks = DtEmp.Rows[i]["OrdRem"].ToString();
                dt.spnoes = DtEmp.Rows[i]["OrdSpRem"].ToString();
        ...
        public PartialViewResult HeaderFotterLoad(long id = 0)
        {
            ...
            sqlParameters[0] = new SqlParameter("@ordid", id);
            sqlParameters[1] = new SqlParameter("@flag", 2);
            DataTable DtEmp = ObjDBConnection.CallStoreProcedure("Update_Ord_sub_Mob_Foot", sqlParameters);
            if (DtEmp != null && DtEmp.Rows.Count > 0)
            {
                model.InqId = Convert.ToInt32(id);
                model.Subject = DtEmp.Rows[0]["OrdSubj"].ToString();
                model.Hedaer = DtEmp.Rows[0]["OrdHeader"].ToString();
                model.Mobile = DtEmp.Rows[0]["Mobile"].ToString();
                model.Footer = DtEmp.Rows[0]["OrdFooter"].ToString();
                if (DtEmp.Rows[0]["OrdSubj"].ToString() == "")
                {
                    SqlParameter[] sqlParameters01 = new SqlParameter[2];
                    sqlParameters01[0] = new SqlParameter("@ordid", (id - 1));
                    sqlParameters01[1] = new SqlParameter("@flag", 2);
                    DataTable DtEmp01 = ObjDBConnection.CallStoreProcedure("Update_Ord_sub_Mob_Foot", sqlParameters01);
                    model.InqId = Convert.ToInt32(id);
                    if (DtEmp01.Rows.Count > 0)
                    {
                        //model.Subject = DtEmp01.Rows[0]["InqSubj"].ToString();
                        //model.Hedaer = DtEmp01.Rows[0]["InqHeader"].ToString();
                        model.Mobile = DtEmp01.Rows[0]["Mobile"].ToString();
                        model.Footer = DtEmp01.Rows[0]["OrdFooter"].ToString();
                    }
                }
            }

            return PartialView("_HeaderFotterOrd", model);
        }

        [HttpPost]
        public IActionResult SaveOrdSubMobFooter(InqHeaderAndFooterModel data)
        {


            List<Extraitem> etrItmObj = new List<Extraitem>();
            SqlParameter[] sqlParameters = new SqlParameter[6];
            sqlParameters[0] = new SqlParameter("@ordid", data.InqId);
            sqlParameters[1] = new SqlParameter("@sub", data.Subject);
            sqlParameters[2] = new SqlParameter("@header", data.Hedaer);
            sqlParameters[3] = new SqlParameter("@footer", data.Footer);
            sqlParameters[4] = new SqlParameter("@mob", data.Mobile);
        ...
        public List<InqueryReportModel> getReportcontent(long id)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow item in ds.Tables[0].Rows)
                {
                    ...
                    inq.totaldiscount = Convert.ToDecimal(item["DiscounAmount"].ToString());
                    inq.totalnet = Convert.ToDecimal(item["NetAmount"].ToString());
                    ...
                    inq.VenueToAddOne = item["VenueToAddOne"].ToString();
                    inq.VenueToAddTwo = item["VenueToAddTwo"].ToString();
                    inq.VenueOneAddTo = item["VenueOneAddTo"].ToString();
                    inq.VenuLink = item["VenueLink"].ToString();
                    inq.IntEveHrs = item["OrtEveHrs"].ToString();
                    inq.city = item["CITY"].ToString();
                    inq.area = item["AccAreaNm"].ToString();
                    // New 

                    inq.header = item["headr"].ToString();


                    if (!string.IsNullOrEmpty(inq.VenuLink))
                        inq.VenuLink = inq.VenuLink.ToLower();
                    Inquery.Add(inq);
                }
            }
        ...
        {
            //1047
            var data = getReportcontent(id);
            string headerUrl = Url.Action("Header", "Home", null, Request.Scheme);
            string footerUrl = Url.Action("Footer", "Home", null, Request.Scheme);


            // Custom switches for Rotativa options
            string customSwitches = $"--print-media-type --footer-right \"Page [page] of [topage]\" --allow {footerUrl} --footer-html {footerUrl} --footer-spacing 0 --header-html {headerUrl}";

            return new ViewAsPdf("OrderPdf", data)
            {
                PageSize = Size.A4,
                PageOrientation = Orientation.Portrait,
                CustomSwitches = customSwitches,
                PageMargins = { Left = 12, Bottom = 50, Right = 12, Top = 15 }
            };
            ...
            foreach (var item in uniqu.ToList())
            {
                ...
                foreach (var item1 in Inquery1)
                {
                    ...
                    if (a == b)
                    {
                        ...
                        AddCellToBody(tableLayout, "Rate :" + Convert.ToDecimal(item1.IntRt).ToString("N2"));
        ...
        public JsonResult GetOrderbyIndNo(int Id)
        {
            ...
            if (DtEmp != null && DtEmp.Rows.Count > 0)
            {
                for (int i = 0; i < DtEmp.Rows.Count; i++)
                {
                    string inputDate = DtEmp.Rows[i]["InqDt"].ToString();
                    string[] dateParts = inputDate.Split('-');
                    string formattedDate = "";
                    if (dateParts.Length == 1)
                    {
                    ...
                    model.QuoNo = Convert.ToInt32(DtEmp.Rows[i]["InqQuoNo"]);
                    models.Add(model);
                }
